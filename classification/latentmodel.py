...
class LatentModel(nn.Module):
    ...
    def h(self, t, z):
        t = t.expand(z.size(0), 1).to("cuda")
        tz = torch.cat([t, z], dim=1)
        return self.h_net(tz)

    def g(self, t, z):
        t = t.expand(z.size(0), 1).to("cuda")
        tz = torch.cat([t, z], dim=1)
        return 0.0*self.g_net(tz)

    def forward(self, xs, ts, ext_ts, adjoint=False, method="euler"):
        ...
        if adjoint:
            ...
            zs, log_ratio = torchsde.sdeint_adjoint(
                self, z0, ext_ts, adjoint_params=adjoint_params, dt=self.dt, logqp=True, method=method)
        else:
